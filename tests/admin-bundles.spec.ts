...
test.describe("Bundles / resources CRUD (API)", () => {
  test("bundle CRUD lifecycle", async () => {
    ...
    try {
      const created = await lessonsApi(api, "post", "/bundles", [{ contentType: "program", contentId: PROGRAM_ID, name: "Zacchaeus Bundle" }]);
  ...
  test("resource CRUD lifecycle (under a bundle)", async () => {
    const api = await getApi("lessons-admin");
    try {
      const bundleResp = await lessonsApi(api, "post", "/bundles", [{ contentType: "program", contentId: PROGRAM_ID, name: "Resource Test Bundle" }]);
      const bundle = (await bundleResp.json())[0];

      const resourceResp = await lessonsApi(api, "post", "/resources", [{ contentType: "program", contentId: PROGRAM_ID, bundleId: bundle.id, name: "Zacchaeus Resource", category: "Handouts" }]);
      ...
      const updateResp = await lessonsApi(api, "post", "/resources", [{ ...resource, name: "Zacchaeus Renamed Resource" }]);
