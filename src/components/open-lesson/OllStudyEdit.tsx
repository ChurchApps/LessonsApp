...
interface Props { study: FeedStudyInterface; updatedCallback: (study: FeedStudyInterface | null, cancelled: boolean) => void; }
...
export function OllStudyEdit(props: Props) {
  ...
  const onValid = (values: AnyRecord) => {
    props.updatedCallback({ ...props.study, id: values.id, name: values.name, image: values.image, description: values.description }, false);
  };

  const handleDelete = () => { if (window.confirm("Are you sure you wish to delete this study?")) props.updatedCallback(null, false); };

  useEffect(() => {
    if (props.study) reset({ id: props.study.id ?? "", name: props.study.name ?? "", image: props.study.image ?? "", description: props.study.description ?? "" });
  }, [props.study, reset]);
