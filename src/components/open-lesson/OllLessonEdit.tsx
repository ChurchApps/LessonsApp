...
import { FeedLessonInterface } from "@/utils";
...
export function OllLessonEdit(props: Props) {
  ...
  const handleSave = () => {
    if (validate()) {
      props.updatedCallback(lesson, false);
    }
  };
  ...
  if (!lesson) return <></>;
    ...
    <>
      <InputBox id="lessonDetailsBox" headerText={props.lesson ? "Edit Lesson" : "Create Lesson"} headerIcon="check" saveFunction={handleSave} cancelFunction={handleCancel} deleteFunction={handleDelete}>
