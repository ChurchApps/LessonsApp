...
import { FeedFileInterface } from "@/utils";
...
export function OlfFileEdit(props: Props) {
  ...
  const handleSave = () => {
    if (validate()) {
      props.updatedCallback(file, false);
    }
  };
  ...
  if (!file) return <></>;
    ...
    <>
      <InputBox id="fileDetailsBox" headerText={props.file ? "Edit File" : "Add File"} headerIcon="check" saveFunction={handleSave} cancelFunction={handleCancel} deleteFunction={handleDelete}>
        ...
        <b>Video Details</b>
        <TextField fullWidth name="thumbnail" label="Thumbnail (optional)" value={file.thumbnail} onChange={handleChange} />
        <TextField fullWidth name="streamUrl" label="Stream URL (optional)" value={file.streamUrl} onChange={handleChange} />
        ...
        {(!file.loop) && <TextField fullWidth name="seconds" label="Seconds" value={file.seconds} onChange={handleChange} />}
