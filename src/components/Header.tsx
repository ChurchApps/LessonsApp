...
export const Header: React.FC = () => {
  const [userName] = React.useState(UserHelper.person?.name?.display || UserHelper.user?.firstName || "Login");
  ...
  const getUser = () => {
    if (!UserHelper.user) return (<Link to="/login">Login</Link>)
    else return (<>{userName} <i className="fas fa-caret-down" ></i ></>);
  }
