...
export default function LessonsPage() {
  ...
  const loadData = async () => {
    ...
    const data = { providerId:params.providerId, programId:program.id, studyId:study.id, lessonId:lesson.id, venueId: lesson.venues[0].id, venues: [venueData] };
    setLessonData(data);
    ...
    <Layout pageTitle={title} metaDescription={selectedVenue.lessonDescription} image={selectedVenue.lessonImage} withoutNavbar>
          ...
          <Container fixed>
            ...
            <Grid container spacing={2}>
              <Grid item md={7} xs={12}>
                <div className="breadcrumb"><Link href={"/" + selectedVenue.programSlug}>{selectedVenue.programName}</Link>: <Link href={"/external/" + lessonData?.providerId + "/" + lessonData?.programId + "/" + lessonData?.studyId }>{selectedVenue.studyName}</Link></div>
      ...
      <Grid container spacing={2}>
        <Grid item md={3} sm={12} style={{backgroundColor:"#FFF"}}>
          <LessonSidebar venues={lessonData?.venues} selectedVenue={selectedVenue} onVenueChange={(v) => { setSelectedVenue(v); }} onPrint={() => { setPrint(Math.random()) } } />
        </Grid>
